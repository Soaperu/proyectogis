...
from worker_functions import get_cuadriculas_regionales
import arcpy
import pandas as pd
...
def main(num_cores, folders, output_dir):
    """
    ...
    # part_func = partial(get_cuadriculas_regionales, some_param=output_dir)
    # response = pool.map(part_func, folders)
    # pool.close()
    # pool.join()
    ...
    for folder in folders:
        get_cuadriculas_regionales(folder, output_dir)
...
if __name__ == '__main__':
    try:
        ...
        datums = arcpy.GetParameterAsText(2).split(';')
        cores = int(3)
        ...
        for datum in datums:
            arcpy.AddMessage(f"DATUM: {datum}")

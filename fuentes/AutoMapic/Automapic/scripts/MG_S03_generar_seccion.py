...
def angle_to_vector(angle):
    """ Convierte un ángulo en radianes a un vector unitario. """
    return math.cos(angle), math.sin(angle)

def producto_cruz_2d(v1, v2):
    """ Calcula el producto cruz en 2D, que es la 'z' del vector resultante. """
...
def dotVectorSeccPogByAngle(ang_sec, ang_pog):
    ...
    # AXB = np.dot(A, B)
    vector_principal = angle_to_vector(ang_sec)
    vector_secundario = angle_to_vector(ang_pog)
    AXB = producto_cruz_2d(vector_principal, vector_secundario)
    return AXB
...
try:
    ...
    for ng in nombres_geograficos:
        ...
        for point_intersect in points_intersect:
            ...
            insertcursor2.insertRow([(first_point.X + r, z), etiqueta, codhoja, 0])
        del insertcursor2
    ...
    aut.add_layer_with_new_datasource(lyer_gpl, name_gpl, st._GDB_PATH_MG, "FILEGDB_WORKSPACE", df_name =None, query=query, zoom=True, scale=50000)

...
try:
    ...
    # 921,431.268  7,896,319.049
    # Coordenadas X de inicio de los componentes de la leyenda
    # x_ini = 194033.875   # Coordenada X inicial de la leyenda (inferior izquierda)
    # y_ini = 8584501.480   # Coordenada Y inicial de la leyenda (inferior izquierda)

    # if tipo_leyenda == '2':
    #     # Coordenadas X de inicio de los componentes de la leyenda
    #     x_ini = x_ini + 5500   # Coordenada X inicial de la leyenda (inferior izquierda)

    # x_ini = 760896.593  
    # y_ini = 8440187.446
    _X_INI_ERATEMA = x_ini   # Coordenada inicial de la columna eratema
    _X_INI_SISTEMA = _X_INI_ERATEMA + w_eratema   # Coordenada X incial de la columna sistema
    ...
    query = "({} = '{}') and ({} = 1) and ({} = {}) and ({} = '{}')".format(_TB_LEYENDA.codhoja, codhoja, _TB_LEYENDA.estado, _TB_LEYENDA.dominio, dominio, _TB_LEYENDA.tipo, tipo_leyenda)
    ...
    for i, row in df_leyenda.iterrows():
        ...
        if i == len(df_leyenda) - 1:
            ...
            unidad = row[_TB_LEYENDA.unidad]
            diagrama_arr_h_unidad = []
        # if (i == len(df_leyenda) - 1) and (unidad not in [un['nombre'] for un in unidad_arr]):
        #     y_max_unidad = y_max_unidad + row[_FIELD_GROSOR]
        #     diagrama_arr_h_unidad.append(row[_FIELD_GROSOR])
        #     unidad_pol = generate_polygon_unidad(sum(diagrama_arr_h_unidad), y_max_unidad)
        #     unidad_arr.append({'nombre': unidad, 'polygon': unidad_pol, 'tipo': _TYPE_BOX['unidad']})
        #     unidad = row[_TB_LEYENDA.unidad]
        #     diagrama_arr_h_unidad = []
    ...
    gpt_mg_label_mfl = arcpy.MakeFeatureLayer_management(gpt_mg_label.path, 'gpl_ann_path_{}'.format(codhoja), query)
    ...
    gpl_aux_mfl = arcpy.MakeFeatureLayer_management(gpl_mg_celd.path, 'gpl_aux_path_{}'.format(codhoja), query)
    ...
    query_view = "({} = '{}') and ({} = 1)".format(_TB_LEYENDA.codhoja, codhoja, _TB_LEYENDA.estado, _TB_LEYENDA.dominio)

    aut.add_layer_with_new_datasource(lyr_ann, gpt_mg_label.name, gpt_mg_label.workspace, 'FILEGDB_WORKSPACE', df_name=None, query=query_view, mxd=mxd)
    aut.add_layer_with_new_datasource(lyr_lin, gpl_mg_celd.name, gpl_mg_celd.workspace, 'FILEGDB_WORKSPACE', df_name=None, query=query_view, mxd=mxd)
    aut.add_layer_with_new_datasource(lyr_pol, gpo_form.name, gpo_form.workspace, 'FILEGDB_WORKSPACE', df_name=None, query=query_view, mxd=mxd)

    layers = arcpy.mapping.ListLayers(mxd, "*{}*".format(gpl_mg_celd.name), adf)
    if layers:
        layers[0].definitionQuery = query
        adf.extent = layers[0].getExtent()
        adf.scale = 50000

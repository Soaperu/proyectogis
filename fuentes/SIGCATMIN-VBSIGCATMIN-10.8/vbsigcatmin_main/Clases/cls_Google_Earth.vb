...
Public Class cls_Google_Earth
    ...
    Public Sub Genera_KML_Google_rese(ByRef m_application As IApplication, ByVal p_Zona As String, ByVal nonbrecapa As String)
        '  NombreCatastro = "Catastro"
        NombreCatastro = nonbrecapa
        ' Dim FileKML As System.IO.FileStream = New System.IO.FileStream(glo_pathTMP & "\Google.kml", FileMode.Create)
        ...
        ListarShapeSelecionados(KML, m_application, p_Zona, nonbrecapa)
        seleccion_opcion = ""
        KML.Close()
        FileKML.Close()
        QuitarSeleccion(m_application)
    ...
    Private Sub KML_DetalleCatastro(ByRef KML As StreamWriter, ByRef p_Layer As IFeatureLayer, ByRef PSAD56_UTM As ISpatialReference, ByRef G_WGS84 As ISpatialReference)
        ...
        If NombreCatastro = "Zona Reservada1" Then
            nameFld = "CODIGO"
            '@CQ
        ElseIf NombreCatastro = "Zona Urbana" Then
            nameFld = "CODIGO"
            '@CQ
        Else
            nameFld = "CODIGOU"
    ...
    Public Function ListarShapeSelecionados(ByRef KML As StreamWriter, ByRef m_application As IApplication, ByVal p_Zona As String, ByVal nombrecapa As String) As Boolean
        Dim theTmpName As String = Nothing
        Dim TheColorKML As String = "ffff0000"
        Dim TheCatastro As Integer = 0
        Dim WGS84Point As IPoint = New Point
        ...
        Select Case p_Zona
            Case 17
                ...
                PSAD56_UTM = SpatRefFact.CreateProjectedCoordinateSystem((32717))
            Case 18
                '  PSAD56_UTM = SpatRefFact.CreateProjectedCoordinateSystem(24878)
                PSAD56_UTM = SpatRefFact.CreateProjectedCoordinateSystem((32718))
            Case 19
                ' PSAD56_UTM = SpatRefFact.CreateProjectedCoordinateSystem(24879)
                PSAD56_UTM = SpatRefFact.CreateProjectedCoordinateSystem((32719))
        End Select
        G_WGS84 = SpatRefFact.CreateGeographicCoordinateSystem(4326)
        pMxDoc = m_application.Document
        pMap = pMxDoc.FocusMap
        ...
        For A = 0 To pMap.LayerCount - 1
            ...
            If pMap.Layer(A).Name = nombrecapa Then
                Afound = True
                ' Return False
                Exit For
            End If
    ...
    Private Function ColorCatastro(ByRef aRec As IFeature)
        'If NombreCatastro = "Zona Reservada1" Then
        '@CQ
        If NombreCatastro = "Zona Reservada1" Or NombreCatastro = "Zona Urbana" Then

            Dim Estado As String = aRec.Value(aRec.Class.FindField("CODIGO"))
            Return "G2"
        Else

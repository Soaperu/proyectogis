...
Public Class cls_excel
    ...
    Private Structure VerXY
        ...
        Dim iconcesionID As Integer
        Dim izonaID As Integer
    End Structure
    ...
    Public Sub Procesa_libredenunexcel(ByVal p_ShapeFile As String, ByVal lodtbDatos As DataTable, ByVal m_application As IApplication)
        ...
        Try
            ...
            SetParam(lodtbDatos)
            Dim n As Integer = 0
            Dim FieldDataCount As Integer = lodtbDatos.Columns.Count ' - 
            ...
            Dim bFound As Boolean = False
            Dim vCodigo1 As String = ""
            Dim v_nombreld As String = ""
            Dim v_zonald As String = ""
            contador = contador + 1
            vCodigoSalvado = lodtbDatos.Rows(0).Item(DataXY.iCodigoID)
            'v_zonald = lodtbDatos.Rows(0).Item(DataXY.izonaID)
            Dim cls_Oracle As New cls_Oracle
            ...
            listadm.Columns.Add("CODIGO", Type.GetType("System.String"))
            ' listadm.Columns.Add("ZONA", Type.GetType("System.String"))
            'listadm.Columns.Add("NOMBRE", Type.GetType("System.String"))
            'Dim dRow1 As DataRow
            ...
            'Try
            'v_codigo = lodtbDatos.Rows(j).Item(DataXY.iCodigoID)
            'v_nombreld = lodtbDatos.Rows(j).Item(DataXY.iconcesionID)
            'v_zonald = lodtbDatos.Rows(j).Item(DataXY.izonaID)
            'If v_codigo <> "" Then
            'dRow1.Item("CODIGO") = v_codigo
            'listadm.Rows.Add(v_codigo)
            '' listadm.Rows.Add(v_codigo)
            ...
            For i As Integer = 0 To lodtbDatos.Rows.Count - 1
                ...
                ' v_zonald = lodtbDatos.Rows(i).Item(DataXY.izonaID)
                cuenta = cuenta + 1
                lodbListaDM = cls_Oracle.F_OBTIENE_DM_UNIQUE(v_codigo, 1)

                If lodbListaDM.Rows.Count >= 1 Then


                    v_zona_dm = lodbListaDM.Rows(0).Item("ZONA").ToString
                    v_vigcat = lodbListaDM.Rows(0).Item("PE_VIGCAT").ToString
                    v_nombre_dm = lodbListaDM.Rows(0).Item("NOMBRE").ToString

                Else
                    If lodbListaDM.Rows.Count = 0 Then
                        lodbListaDM = cls_Oracle.F_OBTIENE_DM_LIBREDENU(v_codigo, 1)
                        If lodbListaDM.Rows.Count = 0 Then
                            MsgBox("Verificar su listado, un DM no se registra en la base de datos", MsgBoxStyle.Critical, "Observación")
                            Exit For

                        Else
                            v_zona_dm = lodbListaDM.Rows(0).Item("ZONA").ToString
                            v_vigcat = lodbListaDM.Rows(0).Item("GRAFICA").ToString
                            v_carta_dm = lodbListaDM.Rows(0).Item("CARTA").ToString
                            v_nombre_dm = "D.M Reducido "
                ...
                ' m_application.Caption = "PROCESO  MASIVO DE LIBRE DENUNCIABILIDAD  :  " & v_codigo & "      " & cuenta.ToString & "  De  " & contador_lista
                m_application.Caption = "PROCESO  MASIVO DE LIBRE DENUNCIABILIDAD  :  " & v_codigo
                cls_catastro.Consulta_Evaluacion_DM_masivo(m_application)
                Dim A As Integer = 2
                ...
                pMxDoc.UpdateContents()
                pMxDoc.ActiveView.Refresh()
                cls_catastro.Genera_Imagen_DM(v_codigo, "libredenunmasivo")
                cls_catastro.Color_AR_Plano(m_application, "Zona Reservada")
                cls_catastro.Genera_Imagen_planopdf(v_codigo, "libredenunmasivo")
                cls_catastro.Borra_Todo_Feature("", m_application)
                cls_catastro.Limpiar_Texto_Pantalla(m_application)
                pMxDoc.UpdateContents()
    ...
    Private Sub SetParam(ByVal lodtbAsigna As DataTable)
        ...
        Next f
    End Sub
    Private Sub SetParam1(ByVal lodtbAsigna As DataTable)
        Dim f As Integer
        For f = 0 To lodtbAsigna.Columns.Count - 1
            'If lodtbAsigna.Columns(f).ColumnName = gloNameEste Then
            'DataXY.iEsteX = f
            ...
            ElseIf lodtbAsigna.Columns(f).ColumnName = "ZONA" Then 'DataXY.sCodigoID Then
                DataXY.izonaID = f
            ElseIf lodtbAsigna.Columns(f).ColumnName = "CONCESION" Then 'DataXY.sCodigoID Then
                DataXY.iconcesionID = f
            End If
        Next f
    End Sub
